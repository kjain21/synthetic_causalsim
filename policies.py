...
class PensieveAgent(Agent):
    def __init__(self, **kwargs):
        super().__init__()
        self.reset, _ = kwargs['env'].reset()
        self.obs_len = self.reset.shape[0]
        self.act_n = kwargs['env'].action_space.n
        #self.obs_len = kwargs['env'].observation_space.shape[0]
        self.normer = Normer()
    ...
    def take_action(self, obs_np):
        #policy_net = torch.jit.script(PensNet(normer(self.obs_len), self.act_n, [32, 16]))
        act = sample_action(self.policy_net, self.normer(obs_np[:2*config.mpc_lookback+3+6]), torch.device('cpu'))
        return act